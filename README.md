...
| Skill | Description |
|-------|-------------|
| [app-store-screenshots](app-store-screenshots/SKILL.md) | Code-driven marketing screenshots for Flutter apps (App Store + Google Play): a `flutter test` harness that renders real/composite screens over seeded data inside a device bezel with captions and an enlarged "hero" element — including the slot-hero system, scaling architecture, pixel-tuning playbook, and localizing the whole set to many languages (per-locale currency + the non-Latin font-fallback gotcha). |
...
| [revenuecat](revenuecat/SKILL.md) | RevenueCat subscription billing for Flutter + NestJS: what can be automated via RC REST API v2 (project, apps, entitlement, offering, packages, products, attach endpoints) vs what requires dashboard clicks (webhook setup, SDK public keys, IAP key, Play service account), full Flutter `BillingService` implementation (`purchases_flutter`, reactive paywall CTA), NestJS `RevenueCatService` + 3-path entitlement resolution + webhook endpoint, sandbox testing checklist, and a table of common errors with fixes — derived from building the full Kachak billing stack end-to-end. |
